...
function CheckoutContent() {
  ...
  if (!user) {
    const redirectPath = `/subscription/checkout?plan=${planId || ''}`;
    return (
      <div className={styles.container}>
        <div className={styles.content}>
          <Flexbox align="center" gap={24} style={{ minHeight: '50vh', paddingBlock: 48 }}>
            <Lock size={48} style={{ opacity: 0.6 }} />
            <Title level={2} style={{ margin: 0, textAlign: 'center' }}>
              {isGlobalPlan ? 'Sign in to continue' : 'Đăng nhập để tiếp tục'}
            </Title>
            <Text style={{ fontSize: 16, maxWidth: 400, textAlign: 'center' }} type="secondary">
              {isGlobalPlan
                ? 'You need to sign in or create an account before completing your purchase.'
                : 'Bạn cần đăng nhập hoặc tạo tài khoản trước khi thanh toán.'}
            </Text>
            {plan && (
              <Card style={{ borderRadius: 12, maxWidth: 400, width: '100%' }}>
                <Flexbox gap={8}>
                  ...
                  <Text strong style={{ fontSize: 18 }}>
                    {isGlobalPlan
                      ? formatPrice(
                          (plan as any).yearlyPriceUSD || (plan as any).monthlyPriceUSD || 0,
                          true,
                        )
                      : formatPrice(plan.yearlyPriceVND || plan.monthlyPriceVND, false)}
                  </Text>
                </Flexbox>
              </Card>
            )}
            <Flexbox gap={12} horizontal style={{ flexWrap: 'wrap', justifyContent: 'center' }}>
              <Button
                onClick={() =>
                ...
                type="primary"
              >
                {isGlobalPlan ? 'Sign In' : 'Đăng nhập'}
              </Button>
              <Button
                onClick={() =>
                  router.push(`/signup?redirect_url=${encodeURIComponent(redirectPath)}`)
                }
                size="large"
                style={{ minWidth: 160 }}
              >
                {isGlobalPlan ? 'Create Account' : 'Tạo tài khoản'}
              </Button>
            </Flexbox>
            <Button icon={<ArrowLeft />} onClick={() => router.back()} type="text">
              {isGlobalPlan ? 'Go Back' : 'Quay lại'}
            </Button>
          </Flexbox>
        </div>
      </div>
