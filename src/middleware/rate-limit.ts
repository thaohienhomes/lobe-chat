...
// ── Daily Tier Rate Limiter — REMOVED ─────────────────────────────────
// Daily tier limits (Tier 2/3 per-day quotas) are now enforced SOLELY by
// atomicAcquireTierSlot() in credits.ts (PostgreSQL atomic UPDATE).
// The previous DailyTierRateLimiter class + Upstash Redis INCR was a
// duplicate counter that caused the tier limit bypass bug.
// Removed in March 2026 — definitive fix.
